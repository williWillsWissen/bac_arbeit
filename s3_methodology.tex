...
Als Erbebniss liefert KRESHMOI eine Liste von Ergebniss-Datensätzen welches neben der ID, Bild-Dimension und Titel noch den Report zu dem entsprechenden Datensatz enthällt.
Die Ergebnisse werden entsprechend der Übereinstimmung mit der Textur aus der Suchanfrage nach ihrer Relevanz sortiert in die Liste eingefügt und weiters wird die Relevanz noch als Kommazahl in den Ergebniss-Datensatz aufgenommen.
\begin{figure}[t]
\begin{lstlisting}[
	language=json,
	firstnumber=1,
